...
PendSV_Handler
    ...
    CBZ     R0, __NO_SAVE                           ; Skip register save at the first time

    SUBS    R0, R0, #0x20                           ; Save remaining regs r4-11 on process stack. r0-r3 have been saved automatically
    STM     R0, {R4 - R11}
    
	LDR     R1, =OSTCBCur                           ; OSTCBCur->OSTCBStkPtr = SP;
    LDR     R1, [R1]
    STR     R0, [R1, #4]                                ; R0 is SP of process being switched out
...
__NO_SAVE
    ...
    ADDS    R0, R0, #0x20                       
    MSR     PSP, R0                                 ; Load PSP with new process stack
        	                                                                            
    ORR     LR, LR, #0x04                           ; Change EXC_RETURN to uses PSP after return
    MOV     R0, #0x1                                ; Config CONTROL reg to uses unprivileged mode after return
    MSR     CONTROL, R0

    MOV     R0, #0 
    MSR 	BASEPRI, R0      	    	            ; enable interruption
